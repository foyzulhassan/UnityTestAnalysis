...
public class GeneralFixture {
	...
	public Map<String,List<AssertCall>> searchForGeneralFixture(ITree root)
	{
		...
		{
			List<ITree> assertlist=TreeNodeAnalyzer.getSearchTypeLabel(testfunc, "attribute", "");
            ...
            if(Objects.equals(funcnamenode.getLabel(), "Setup") || funcnamenode.getLabel().equals("SetUpTest")){
                System.out.println("Setup found");
            }
            if (assertlist.size()>0){
                for ( ITree x:assertlist){
                    String finalClasstestfunc = classtestfunc;
                    x.getChildren().forEach(s1 -> s1.getChildren().forEach(
                           s2 ->
                                   //TODO: continue working on attribute choiche here
//                               if (Objects.equals(s2.getLabel(), "Test")){
                                   System.out.println(s2.getLabel())
